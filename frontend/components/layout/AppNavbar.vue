  ...
  <header class="sticky top-0 z-40 border-b border-line bg-white/90 backdrop-blur">
    <div class="mx-auto flex max-w-7xl items-center justify-between gap-4 px-4 py-4 sm:px-6">
      ...
      <nav class="hidden items-center gap-6 text-sm font-medium text-muted md:flex">
        <NuxtLink to="/" class="transition hover:text-pink" active-class="text-pink">Inicio</NuxtLink>
        <NuxtLink to="/businesses" class="transition hover:text-pink" active-class="text-pink">Emprendimientos</NuxtLink>
      </nav>
      ...
      <div class="flex items-center gap-2">
        ...
        <div v-else class="relative">
          <button
            class="flex items-center gap-2 rounded-full border border-line px-3 py-1.5 text-sm font-medium text-ink hover:border-pink"
            @click="menuOpen = !menuOpen"
          >
            <span class="flex h-7 w-7 items-center justify-center rounded-full bg-pink-light text-xs font-semibold text-pink">
          ...
          >
            <NuxtLink v-if="authStore.isAdmin" to="/admin" class="flex items-center gap-2 rounded-xl2 px-3 py-2 text-sm text-ink hover:bg-pink-light">
              <AppIcon name="settings" :size="16" /> Panel admin
            </NuxtLink>
            <NuxtLink to="/dashboard" class="flex items-center gap-2 rounded-xl2 px-3 py-2 text-sm text-ink hover:bg-pink-light">
              <AppIcon name="briefcase" :size="16" /> Mi panel
            </NuxtLink>
            <NuxtLink to="/dashboard/settings" class="flex items-center gap-2 rounded-xl2 px-3 py-2 text-sm text-ink hover:bg-pink-light">
