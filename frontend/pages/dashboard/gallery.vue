...
<template>
  <div>
    ...
    <p class="mt-1 text-sm text-muted">Muestra fotos de tu trabajo o tu espacio (máximo {{ MAX_IMAGES }}).</p>
    ...
    <template v-else>
      <div v-if="images.length < MAX_IMAGES" class="mt-6">
        <ImageUploader label="Agregar foto" :upload-fn="uploadFn" />
      </div>
      <p v-else class="mt-6 text-sm text-muted">
        Llegaste al máximo de {{ MAX_IMAGES }} fotos. Elimina una para subir otra.
      </p>

      <LoadingSpinner v-if="loading" />
      <template v-else-if="images.length">
        <p class="mt-4 text-xs font-medium text-muted">{{ images.length }}/{{ MAX_IMAGES }} fotos</p>
